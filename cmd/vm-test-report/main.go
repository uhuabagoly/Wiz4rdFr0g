...
func main() {
	...
	w := csv.NewWriter(f)
	_ = w.Write([]string{"catalog_index", "name", "resolved_id", "install_ok", "install_verified", "uninstall_ok", "uninstall_verified", "final_status", "coverage_status", "root_cause", "failure_stage", "failure", "skip_reason"})
	for _, r := range rows {
		_ = w.Write([]string{strconv.Itoa(r.CatalogIndex), r.Name, r.ResolvedID, strconv.FormatBool(r.InstallOK), strconv.FormatBool(r.InstallVerified), strconv.FormatBool(r.UninstallOK), strconv.FormatBool(r.UninstallVerified), r.FinalStatus, r.CoverageStatus, r.RootCause, r.FailureStage, r.Failure, r.SkipReason})
	}
	w.Flush()
	_ = f.Close()
