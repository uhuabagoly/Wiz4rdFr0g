...
func vmActionsEnvironment(idx int) (json.RawMessage, error) {
	...
	if os.Getenv("WIZ4RDFR0G_TEST_RUN_ID") != wantRun || os.Getenv("WIZ4RDFR0G_VM_ID") != fmt.Sprintf("%s-%d", wantRun, idx) {
		return nil, fmt.Errorf("Actions run/VM identity mismatch")
	}
	b, err := os.ReadFile(os.Getenv("WIZ4RDFR0G_ENVIRONMENT_EVIDENCE"))
	if err != nil {
		return nil, err
	}
	var e struct {
	...
	if e.Status != "READY" || e.VMID != os.Getenv("WIZ4RDFR0G_VM_ID") || e.Commit != buildGitCommit {
		return nil, fmt.Errorf("preflight missing or mismatched")
	}
